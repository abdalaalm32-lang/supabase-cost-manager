...
/** color logic for an expense item (higher = worse) */
...
/** color logic for sales / profitability (higher = better) */
...
export const PeriodComparisonDialog: React.FC<Props> = ({ open, onOpenChange, periods, branches, defaultPeriodId }) => {
  ...
  const analysis = useMemo(() => {
    ...
    const labels = Array.from(new Set([...aBreak.map(x => x.label), ...bBreak.map(x => x.label)]));
    const rows = labels.map(label => {
      ...
      return { label, a: av, b: bv, diff, pct };
    });
    ...
    if (totalDiffPct >= 20) {
      recs.push({ type: "danger", text: `إجمالي المصاريف زاد بنسبة ${totalDiffPct.toFixed(1)}% — راجع البنود اللي زادت بشكل كبير وحاول تخفيضها.` });
    } else if (totalDiffPct >= 10) {
      recs.push({ type: "warning", text: `إجمالي المصاريف زاد بنسبة ${totalDiffPct.toFixed(1)}% — انتبه لأي زيادة إضافية قد تأثر على الربحية.` });
    } else if (totalDiffPct <= -5) {
      recs.push({ type: "success", text: `إجمالي المصاريف قل بنسبة ${Math.abs(totalDiffPct).toFixed(1)}% — أداء ممتاز في تقليل التكاليف.` });
    }
    for (const r of increased) {
      if (r.pct >= 25) {
        recs.push({ type: "danger", text: `بند "${r.label}" زاد بنسبة ${r.pct.toFixed(1)}% (+${fmt(r.diff)}) — يحتاج مراجعة عاجلة.` });
      }
    }
    const salesDiffPct = aSales !== 0 ? ((bSales - aSales) / Math.abs(aSales)) * 100 : 0;
    if (salesDiffPct <= -10) {
      recs.push({ type: "warning", text: `المبيعات الشهرية المتوقعة قلت بنسبة ${Math.abs(salesDiffPct).toFixed(1)}% — راجع السعة، معدل الدوران، أو متوسط الفاتورة.` });
    }
    if (bPct - aPct >= 5) {
      recs.push({ type: "danger", text: `نسبة المصاريف الغير مباشرة من المبيعات ارتفعت من ${aPct.toFixed(1)}% إلى ${bPct.toFixed(1)}% — ده هيقلل صافي الربح.` });
    } else if (bPct - aPct <= -3) {
      recs.push({ type: "success", text: `نسبة المصاريف الغير مباشرة قلت من ${aPct.toFixed(1)}% إلى ${bPct.toFixed(1)}% — تحسن واضح في الكفاءة.` });
    }
    if (recs.length === 0) {
      recs.push({ type: "info", text: "مفيش تغييرات جوهرية بين الفترتين — الأداء مستقر." });
    }

    return { rows, aSales, bSales, aTotal, bTotal, aPct, bPct, increased, decreased, recs };
  }, [A, B]);
        ...
        {sorted.length < 2 ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium mb-2 block">الفترة السابقة (A)</label>
                <Select value={aId} onValueChange={setAId}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {sorted.map(p => (
                      <SelectItem key={p.id} value={p.id}>
                        {p.name} — {branchName(p.branch_id)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">الفترة الحالية (B)</label>
            ...
            {A && B && aId === bId && (
              <div className="p-4 border border-yellow-500/40 bg-yellow-500/10 rounded-lg text-sm text-yellow-700 dark:text-yellow-400">
                ⚠️ اخترت نفس الفترة في الاتنين — اختر فترتين مختلفتين عشان تشوف المقارنة.
              </div>
            )}
            ...
            {A && B && aId !== bId && analysis && (
              <>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-1">
                      ...
                      <div className="pt-2 border-t">
                        <ChangeCell a={analysis.aPct} b={analysis.bPct} />
                      </div>
                    </CardContent>
                  </Card>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <Card>
                    <CardHeader className="pb-2"><CardTitle className="text-sm text-red-500">🔺 أعلى 3 بنود زادت</CardTitle></CardHeader>
                    <CardContent>
                  ...
                  <Card>
                    <CardHeader className="pb-2"><CardTitle className="text-sm text-emerald-600">🔻 أعلى 3 بنود قلت</CardTitle></CardHeader>
                    <CardContent>
